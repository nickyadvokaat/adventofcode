...
class Challenge2 {
    ...
    class func run() {
        print("The checksum is: \(checksumMinMax(forSpreadSheet: input))")
        print("The even divisible checksum is: \(checksumEvenDivisible(forSpreadSheet: input))")
    }

    private class func checksumMinMax(forSpreadSheet spreadSheet: [[Int]]) -> Int {
        return spreadSheet.reduce(0, {$0 + $1.max()! - $1.min()!})
    }

    private class func checksumEvenDivisible(forSpreadSheet spreadSheet: [[Int]]) -> Int {
        return spreadSheet.reduce(0, { sum, row in
            var sumOfDivisions = 0
            for i in 0 ..< row.count - 1 {
                for j in i + 1 ..< row.count {
                    let v1 = row[i]
                    let v2 = row[j]
                    if v1 % v2 == 0 {
                        sumOfDivisions += v1 / v2
                    }
                    if v2 % v1 == 0 {
                        sumOfDivisions += v2 / v1
                    }
                }
            }
            return sum + sumOfDivisions
        })
    }
}
